...
namespace MIRCO
{
  ...
  namespace Input
  {
    ...
    struct Optimization_Algorithm_NNLS
    {
    ...
    struct Solver_Parameters
    {
      ...
      std::optional<Optimization_Algorithm_NNLS> optimization_algorithm;
    };  // -> all
    ...
    struct Geometrical_Parameters
    {
      double Delta;
      optional<std::string>
          topology_Type;  // if not given, auto-detection precedence is file then RMG

      double Topology_LateralLength;

      std::variant<Topology> topology;
    };
  ...
  ViewMatrix_h CreateSurfaceFromStruct(Input::);
